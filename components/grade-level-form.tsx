...
export function GradeLevelForm({ gradeLevel, existingGradeLevels, onSave, onCancel }: GradeLevelFormProps) {
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) {
      ...
      existingGradeLevels.some(
        (grade) => grade.name.toLowerCase() === formData.name.toLowerCase() && grade.id !== gradeLevel?.id,
      )
    ) {
